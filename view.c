...
static int frame_ev(struct session *ses, struct f_data_c *fd, struct links_event *ev)
{
	...
	if (ev->ev == EV_KBD && !(ev->y & KBD_PASTING)) {
		...
		else if (ev->x == 'j') rep_ev(ses, fd, down, 0); /* Paul Was Here */
		else if (ev->x == 'k') rep_ev(ses, fd, up, 0); /* Paul Was Here */
		...
		/* Copy current link to clipboard */
            ...
            /* paul: removed 'l' event for HJKL movement */
		...
		else if ((ev->x == 'l' && !ses->term->spec->braille) || ev->x == KBD_ENTER) {
            /* Paul: replaced KBD_RIGHT with 'l' */
			x = enter(ses, fd, 0);
		} else if (ev->x == '*') {
			ses->ds.images ^= 1;
...
void send_event(struct session *ses, struct links_event *ev)
{
	...
	if (ev->ev == EV_KBD) {
		...
		if (ev->x == 'h' && !ses->term->spec->braille) {
			go_back(ses, 1); /* PAUL WAS HERE */
			goto x;
		}
