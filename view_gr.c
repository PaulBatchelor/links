...
int g_frame_ev(struct session *ses, struct f_data_c *fd, struct links_event *ev)
{
	...
	switch ((int)ev->ev) {
		...
		case EV_KBD:
			...
			if (ev->x == KBD_DEL || (upcase(ev->x) == 'N' && ev->y & KBD_CTRL) || (ev->x == KBD_DOWN && !(ev->y & (KBD_CTRL | KBD_ALT)))) {
                /* paul: 'l' to KBD_DOWN*/
				return scroll_v(fd, 32);
			}
			if (ev->x == KBD_INS || (upcase(ev->x) == 'P' && ev->y & KBD_CTRL) || (ev->x == KBD_UP && !(ev->y & (KBD_CTRL | KBD_ALT)))) {
                /* paul: 'p' to KBD_UP */
				return scroll_v(fd, -32);
			}
			if (ev->x == 'j') {
